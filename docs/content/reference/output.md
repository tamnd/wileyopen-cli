...
describes what users get. Pick a format with `-o`, or let wileyopen choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
wileyopen <command> -o table   # aligned columns for reading
wileyopen <command> -o jsonl   # one JSON object per line, for piping
wileyopen <command> -o json    # a single JSON array
wileyopen <command> -o csv     # spreadsheet friendly
wileyopen <command> -o tsv     # tab-separated
wileyopen <command> -o url     # just the URL column
wileyopen <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
wileyopen <command> --fields id,title,url
```
...
```bash
wileyopen <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
wileyopen <command>            # a table, because this is a terminal
wileyopen <command> | wc -l    # JSONL, because this is a pipe
```
